...
class CommandManager {
    constructor(yeri) {
        ...
        this.modulesMgr = new ModulesManager();
        this.permissionsMgr = new PermissionsManager(yeri.options.discord.ownerId);
        this.lastCommands = new Map();
    }

    sync() {
        this.permissionsMgr.sync(this.yeri.db);
        this.modulesMgr.sync(this.yeri.db);
    }
    ...
    handleMessage(message) {
        ...
        let userPermission = this.permissionsMgr.getUserPermission(message.author.id, message.guild);
        ...
        for (let module of this.modules.values()) {
            for (let command of module.commands) {
                command.matches(module, cmdReq, (match, obstacle) => {
                    ...
                    if (this.modulesMgr.getModuleState(module.name) == false) {
