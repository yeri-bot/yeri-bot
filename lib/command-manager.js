    ...
    handleMessage(message) {
        ...
        for (let module of this.modules.values()) {
            for (let command of module.commands) {
                command.matches(cmdRequest, (match, obstacle) => {
                    if (!match) {
                    ...
                    command.callback.call(module, this.botInstance, res, cmdRequest.parameters, message.content, message.author, message.channel, message.guild);
                    executed = true;
                });
                if (executed) break;
            }
